    ...
    return BlocListener<TaskController, TaskStatus>(
      ...
      listener: (context, state) {
        if (state == TaskStatus.success) {
          Navigator.pop(context); // isso faz com que caso True, sai da tela.
              ...
              children: [
                ...
                SizedBox(
                  width: MediaQuery.of(context)
                      .size
                      .width, //esse cod ele serve para se ajudar em qualquer tela.
                  height: 49,
                  child: ButtonWithLoader<TaskController, TaskStatus>(
                    bloc: widget.controller,
                    selector: (state) =>
                        state ==
                        TaskStatus.loading, // pra mostrar o botao de load.
                    label: 'Salvar',
                    onPressed: () {
                      final formValid =
                          _formKey.currentState?.validate() ?? false;
                      if (formValid) {
                        final duration = int.parse(_durationEC
                            .text); // esse int .parse impede a digitação de texto, aceita apenas número.
